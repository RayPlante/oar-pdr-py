...
class BagBuilder(PreservationSystem):
    ...
    def disconnect_logfile(self, logfile=None):
        ...
        """
        files = list(self._log_handlers.keys())
        if not logfile:
            if not files:
    ...
    def _extend_file_list(self, filelist, param):
        ...
        if extras:
            ...
            if hasattr(extras, '__iter__'):
                bad = [f for f in extras if not isinstance(f, str)]
                if bad:
                    self.log.warning("Ignoring entries in config param, "+param+
                                     ", with non-string type: " + str(bad))
                    extras = [f for f in extras if isinstance(f, str)]
    ...
    def add_ds_pod(self, pod, convert=True, savefilemd=True):
        ...
        """
        if not isinstance(pod, (str, Mapping)):
    ...
    def _write_baginfo_data(self, data, infofile=None, overwrite=False):
        ...
        with open(infofile, mode, encoding='utf-8') as fd:
            for name, vals in list(data.items()):
                if isinstance(vals, str) or \
                   not isinstance(vals, Sequence):
                    vals = [vals]
                for val in vals:
            ...
            with open(os.path.join(self.bagdir, "about.txt"), 'w', encoding='utf-8') as fd:
                ...
                if 'authors' in nerdm:
                    ...
                    if len(auths) > 0:
                        ...
                        print( re.sub(r'=', ' ', textwrap.fill(aus)),
                               file=fd )

                        i=1
                        for affil in affils:
                           print(textwrap.fill("[{0}] {1}".format(i, affil)), file=fd)
                ...
                if 'contactPoint' in nerdm:
                    ...
                    if cp.get('fn') and cp.get('hasEmail'):
                        ...
                        print("Contact: {0} ({1})".format(cp['fn'], aus), file=fd)
                    else:
